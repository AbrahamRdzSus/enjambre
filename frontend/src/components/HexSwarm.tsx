  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} width="100%" role="img"
      ...
      {nodes.map((n) => (
        <g key={`n-${n.a.name}`}>
          <motion.polygon
            points={hexPoints(n.x, n.y, size * 0.046)}
            fill="#141020"
            stroke={colorFor(n.a)}
            strokeWidth="1.6"
            strokeOpacity={n.a.enabled ? 1 : 0.4}
            filter={n.a.enabled ? 'url(#hs-glow)' : undefined}
            animate={reduce || !n.a.enabled ? undefined : { opacity: [0.85, 1, 0.85] }}
            transition={reduce ? undefined : { duration: 3, repeat: Infinity, ease: 'easeInOut' }}
          />
          <circle cx={n.x} cy={n.y} r={size * 0.012} fill={colorFor(n.a)}
            opacity={n.a.enabled ? 1 : 0.4} />
          <text x={n.x} y={n.y + size * 0.085} textAnchor="middle"
            fontSize={size * 0.026} fill="#a99fc7"
